function [V,dm,flag] = poly_Lyapunov(a0, a1, xi, Wa, Wco, delta, e, dm)
...
V = transpose(Mvar(:))*(x(1:floor(size(x,1)/2),1) - x(floor(size(x,1)/2)+1:size(x,1)-1,1));
